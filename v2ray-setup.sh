...
domain_check() {
    ...
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        ...
        echo -e "${Error} ${RedBG} Harap pastikan bahwa rekam A yang benar ditambahkan ke nama domain, jika tidak, V2ray tidak dapat digunakan secara normal ${Font}"
